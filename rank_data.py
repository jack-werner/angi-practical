...
def calc_score(df: pd.DataFrame):
    ...
    return (
        df["days_accredited"].fillna(0)
        + df["numeric_bbb"].fillna(0)
        + df["customer_rating_avg"].fillna(0)
        + df["days_in_operation"].fillna(0)
        - df["complaints_l12m"].fillna(0)
        - df["complaints_l36m"].fillna(0)
    )
...
if __name__ == "__main__":
    ...
    df.to_csv("outputs/rankings.csv")
